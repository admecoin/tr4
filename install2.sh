...
installNodeAndYarn () {
    ...
    sudo chown -R explorer:explorer /explorer/.config
    clear
}
...
installTimeismoney () {
    ...
    cd
    rm timeismoney*
    mkdir -p explorer/.timeismoney/mainnet
    cat > explorer/.timeismoney/mainnet/timeismoney.conf << EOL
rpcport=11334
rpcuser=$rpcuser
...
Type=forking
User=explorer
WorkingDirectory=explorer
ExecStart=explorer/bin/timeismoneyd -datadir=explorer/.timeismoney
ExecStop=explorer/bin/timeismoney-cli -datadir=explorer/.timeismoney stop
Restart=on-abort
[Install]
...
installBlockEx () {
    ...
    git clone https://github.com/admecoin/tr3 explorer/blockex
    cd explorer/blockex
    yarn install
    cat > explorer/blockex/config.js << EOL
const config = {
  'api': {
...
EOL
    ...
    clear
    cat > mycron << EOL
*/1 * * * * cd explorer/blockex && ./script/cron_block.sh >> ./tmp/block.log 2>&1
*/1 * * * * cd explorer/blockex && /usr/bin/nodejs ./cron/masternode.js >> ./tmp/masternode.log 2>&1
*/1 * * * * cd explorer/blockex && /usr/bin/nodejs ./cron/peer.js >> ./tmp/peer.log 2>&1
*/1 * * * * cd explorer/blockex && /usr/bin/nodejs ./cron/rich.js >> ./tmp/rich.log 2>&1
*/5 * * * * cd explorer/blockex && /usr/bin/nodejs ./cron/coin.js >> ./tmp/coin.log 2>&1
EOL
    crontab mycron
    rm -f mycron
